#!/bin/bash
###############################################################################
# build.sh — Compile MediaMagic.app from the Swift sources.
#
# Run on macOS 13+ with Xcode Command Line Tools installed (xcode-select --install).
# Produces:  ./builds/<semver>+<build_number>/MediaMagic.app
#
# Usage:
...
SOURCES="${SCRIPT_DIR}/Sources/MediaMagic"
...
ASSET_NAME="MediaMagic-${BUILD_ID}-macOS.zip"
...
EOF
    if [[ -n "${MEDIA_MAGIC_PRERELEASE:-}" ]]; then
...
PREREL
    ...
    # Set MEDIA_MAGIC_PRERELEASE=1 to mark the GitHub Release as a pre-release
...
swiftc "${SWIFT_FLAGS[@]}" \
    -o "${APP}/Contents/MacOS/MediaMagic" \
    "${SOURCES}"/*.swift
...
<dict>
    ...
    <string>Media Magic</string>
    <key>CFBundleDisplayName</key>
    <string>Media Magic</string>
    <key>CFBundleIdentifier</key>
    <string>com.local.mediamagic</string>
    <key>CFBundleVersion</key>
    <string>${BUILD_NUMBER}</string>
    <key>CFBundleShortVersionString</key>
    <string>${MARKETING_VERSION}</string>
    <key>CFBundleExecutable</key>
    <string>MediaMagic</string>
    ...
    <string>Media Magic — local-only video conversion orchestrator.</string>
...
echo "  Binary: $(file "${APP}/Contents/MacOS/MediaMagic" | head -1)"
